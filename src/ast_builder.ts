...
export class AstBuilder {
    ...
    map_to_ordered(): OrderedToken[] {
        ...
        for (let i = 0; i < this.tokens.length; ++i) {
            ...
            if (O_BRACES.includes(tok.type)) {
                ...
                res[bracket_stack.pop()!]!.matching_br_pos = i;
                cur_depth -= 1;
            }
            else {
    ...
    build(): AstNode {
        ...
        const root: AstNode = first_tok.is_bracket ? new AstBracketNode(first_tok, { l_b: first_tok.pos, r_b: first_tok.matching_br_pos! }, null, null, null, this.context)
            : new AstNode(first_tok, null, null, this.context);
        for (const tok of ordered_tokens.slice(1)) {
            root.insert_node(tok.is_bracket ? new AstBracketNode(tok, { l_b: tok.pos, r_b: tok.matching_br_pos! }, null, null, null, this.context)
